...
const Card = ({
  ...
  const API_ENDPOINT = `http://87.107.105.201:8000/advertisement/filter/?${carCategory ? `&car_category=${carCategory}` : ""
    }${carColor ? `&car_color=${carColor}` : ""}${startDate ? `&start_date=${startDate}` : ""
    }${endDate ? `&end_date=${endDate}` : ""}${state ? `&state=${state}` : ""}${lowerPrice ? `&lower_price=${lowerPrice}` : ""
